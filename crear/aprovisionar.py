...
class aprovisionar:
    ...
    #APROVISIONAR ZTE
    def zte(ip_olt:str , ordenes:list):
        user="AprovCantv" 
        password="Apr0vC@ntv#21$"
        ...
        with engine.connect() as connection:
            ...
            while i<result_rows:
                ...
                devices_access.send("interface gpon-olt_1/"+slot+"/"+puerto+"\n")
                time.sleep(1)
                devices_access.send("onu "+ont_id+" type "+srvprofile+" sn "+serial_modem+"\n")
                time.sleep(1)
                devices_access.send("onu "+ont_id+" profile line "+line+" remote "+lineprofile+"\n")
                time.sleep(1)                
            ...
            while x<result_rows: 
                ...
                devices_access.send("interface gpon-onu_1/"+slot+"/"+puerto+":"+ont_id+"\n")

                devices_access.send("max-mac-learn 4 vport 1\n")

                devices_access.send("switchport mode hybrid vport 1\n")

                devices_access.send("service-port 1 vport 1 user-vlan untag vlan "+c_vlan+" svlan "+vlan+" ingress "+vs+" egress "+vb+"\n")

                devices_access.send("service-port 1 description "+descripcion+"\n")
                devices_access.send("exit\n")
